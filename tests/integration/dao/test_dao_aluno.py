...
class TestDaoAluno:
    def __setup_aluno(
        ...
        aluno, dao = self.__salva_aluno_banco(cria_banco, id, nome, cr, situacao)
        actual = dao.pega_tudo()
        return actual, aluno

    def __salva_aluno_banco(self, cria_banco, id, nome, cr, situacao):
        aluno = Aluno(nome)
        aluno.define_cr(cr)
        aluno.define_id(id)
    ...
    def __setup_lista_alunos(
        self, cria_banco, id_=3, situacao=Situacao.em_curso.value, cr=0, nome=None
    ):
        self.__setup_aluno(cria_banco)
        self.__setup_aluno(cria_banco)
        expected, actual = self.__setup_aluno(
            cria_banco, id=id_, situacao=situacao, cr=cr, nome=nome
        )
    ...
    def test_aluno_pode_ser_atualizado_banco(
        self, cria_banco, cria_massa_dados, cria_curso_com_materias
    ):
        cria_massa_dados
        id_ = "1"
        aluno = DaoAluno(Aluno(), cria_banco).pega_por_id(id_)
    ...
    def test_lista_alunos_recuperada_banco_com_situacao_correta(self, cria_banco):
        ...
        expected, actual = self.__setup_lista_alunos(cria_banco, situacao=situacao)
    ...
    def test_coeficiente_rendimento_objeto_aluno_recuperado_banco(self, cria_banco):
        actual, expected = self.__setup_aluno(cria_banco)
        assert (
