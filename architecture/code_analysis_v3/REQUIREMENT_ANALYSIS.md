...
- Allowed Actions for each object state not defined in the diagrams: found while implementing the tests to `Teacher.set_gss`. For each State it is important to define the allowed and not allowed actions. For example, Teacher can set GSS only when working 
...
- Student can be enrolled in a new course when ends (Approved/Fail) the previous one: found while reviewing the State Machines.
- Rollback/Blocking state change for unnauthorized actions not defined: found while reviewing the State Machines. Considering that a Course can have many 'in progress' and 'locked' Students. If a Student changes from 'locked' to 'in progress', then the number of 'in progress' Students in a Course increases. The Course can be 'full' of 'in progress' students, but still have other 'locked' Students. In this situation, no 'locked' Student could be changed to 'in progress'. Otherwise, the maximum number of 'in progress' Students would be exceeded. According to the architecture, there is no rollback or blocker mecanism to avoid the unauthorized change of the Student state. The same logc can be applied to Subjects related to Students, Teacher related to Subjects and so on. The solution will be explored and documented.
